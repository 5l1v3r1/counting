...
def getLabelsCells(args, markers, img_pad, base_x, base_y, stride, scale):
...
def getTrainingExampleCells(args, img_raw, framesize_w, framesize_h, labelPath, base_x,  base_y, stride, scale):
    ...
    labels, count  = getLabelsCells(args, markers, img_pad, base_x, base_y, args.stride, scale)
...
def main():
    ...
    img, lab, count = getTrainingExampleCells(args, img_raw, framesize_w, framesize_h, labelPath, x, y, args.stride, args.scale)
    print "count", count
            ...
            for base_x in range(0,img_raw.shape[0],framesize_h):
                for base_y in range(0,img_raw.shape[1],framesize_w):
                    if (img_raw.shape[1] - base_y < framesize_w) or (img_raw.shape[0] - base_x < framesize_h):
                        ...
                        continue
                    img, lab, count = getTrainingExampleCells(args, img_raw, framesize_w, framesize_h, labelPath, base_y, base_x, args.stride, args.scale)
                    print "count ", count
